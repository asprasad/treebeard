...
def RunSingleTest_Hummingbird(modelJSONPath, csvPath, regressor=True) -> float:
  if regressor:
    xgb_model = xgb.XGBRegressor()
  else:
    xgb_model = xgb.XGBClassifier()
  xgb_model.load_model(modelJSONPath)
  hb_model = convert(xgb_model, 'pytorch', device='cuda')
...
def RunTestOnSingleModelTestInputs_RAPIDs(modelName : str) -> None:
  ...
  return RunSingleTest_Hummingbird(modelJSON, csvPath)


if __name__ == "__main__":
  RunTestOnSingleModelTestInputs_RAPIDs("abalone")
