...
loop is mapped to \op{grid.x}).
The schedule also tiles the \op{tree} loop into a nest of two loops with indices $t0$ and $t1$. 
It then additionally parallelizes across the $t1$ loop.
...
These schedules just use a combination of 4-8 constructs and already generate schedules 
that are different from what library based systems like XGBoost, Tahoe and RAPIDs FIL use. 
As one can imagine several other schedules with different trade-offs can be generating using the constructs.
