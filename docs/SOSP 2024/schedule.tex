\section{\Treebeard{}'s Scheduling Language}
\label{sec:schedule}
% As established in Section \ref{sec:motivation}, there are several 
% configurations and optimizations strategies for decision 
% tree inference. The best one to use varies significantly across
% models, batch sizes and hardware platforms. Therefore, designing
% any one hard-coded strategy is not feasible as this would 
% make portable performance impossible. 
...
to build automatic schedulers (Section \ref{sec:exploring}).
...
\op{tree} that are used to represent the batch and tree loops\footnote{
...
The compiler internally represents loops (index variables) 
...
\begin{table}[htb]
  ...
  \begin{tabularx}{\linewidth}{c | l | l}
   ...
   with the specified tile size. Resulting loops will be represented by \textbf{outer} and \textbf{inner}.}} \\
                                               ...
                                               &  \textbf{tileSize} &  \\
   \midrule
   \multirow{4}{*}{\texttt{split}} & \textbf{indexVar}  & \multirow{4}{*}{\parbox{0.55\linewidth}{Fiss the loop represented by \textbf{indexVar}
   at iteration \textbf{splitIter}. Resulting loops will be represented by \textbf{first} and \textbf{second}.}} \\
                                               &  \textbf{first} & \\
                                               &  \textbf{second} &  \\
                                               &  \textbf{splitIter} &  \\
   \midrule
   \multirow{3}{*}{\texttt{reorder}} & \textbf{indices[]}  & \multirow{3}{*}{\parbox{0.55\linewidth}{Permute loops corresponding to the specified index variables.
  ...
  %                                              &   &  \\
   \midrule
   \multirow{2}{*}{\texttt{gpuDimension}} & \textbf{indexVar}  & \multirow{2}{*}{\parbox{0.55\linewidth}{Map the passed index variable to a dimension of either the grid
   or thread block.}} \\
                                               & \textbf{gpuDim}  & \\
...
\begin{table}[htb]
  ...
  \begin{tabularx}{\linewidth}{c | l | l}
   ...
   \textbf{Directive} & \textbf{Inputs} & \textbf{Description} \\
   \midrule
   \multirow{3}{*}{\texttt{cache}} & \textbf{indexVar}  & \multirow{3}{*}{\parbox{0.55\linewidth}{Cache the working set of one iteration of the specified loop. 
   ...
   \midrule                                               
   \multirow{2}{*}{\texttt{interleave}} & \textbf{indexVar}  & \multirow{2}{*}{\parbox{0.55\linewidth}{Interleave tree walks within the specified 
...
\begin{table}[htb]
  ...
  \begin{tabularx}{\linewidth}{c | l | l}
   ...
   parallel iterations of the specified loop.}} \\
                                               ...
                                               &  &  \\
   \midrule
   \multirow{2}{*}{\texttt{sharedReduce}} & \textbf{indexVar}  & \multirow{2}{*}{\parbox{0.55\linewidth}{Specifies that intermediate
...
% Before presenting the examples, we note that \Treebeard{}'s 
...
In the \emph{shared data} strategy~\cite{Tahoe}, a thread block walks all the trees 
