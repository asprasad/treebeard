...
bool Test_LoadTileThresholdOp_DoubleInt32_TileSize1(TestArgs_t &args) {
  ...
  auto module = mlir::ModuleOp::create(
      location,
      llvm::StringRef("Test_LoadTileThresholdOp_DoubleInt32_TileSize1"));

  int64_t length = 5;
  int64_t shape[] = {length};
  auto tileType = decisionforest::TiledNumericalNodeType::get(
      builder.getF64Type(), builder.getI32Type(), 1);
  auto inputMemrefType = MemRefType::get(shape, tileType);
  auto outputMemrefType = MemRefType::get(shape, builder.getF64Type());
  auto functionType = builder.getFunctionType(
      {inputMemrefType, outputMemrefType}, builder.getI32Type());

  auto func = builder.create<mlir::func::FuncOp>(
      location, std::string("TestFunction"), functionType);
  func.setPublic();
  auto &entryBlock = *(func.addEntryBlock());
  builder.setInsertionPointToStart(&entryBlock);

  auto lengthConstant =
      builder.create<arith::ConstantIndexOp>(location, length);
  ...
  auto batchLoop = builder.create<scf::ForOp>(
      location, zeroConst, lengthConstant,
      oneIndexConst /*, static_cast<Value>(memrefResult)*/);
  ...
  auto threshold = builder.create<decisionforest::LoadTileThresholdsOp>(
      location, builder.getF64Type(), static_cast<Value>(inputMemref),
      static_cast<Value>(i), static_cast<Value>(zeroConst));

  builder.create<memref::StoreOp>(
      location, TypeRange({}), static_cast<Value>(threshold), outputMemref, i);

  builder.setInsertionPointAfter(batchLoop);
  auto retVal =
      builder.create<arith::ConstantIntOp>(location, 0, builder.getI32Type());
  builder.create<mlir::func::ReturnOp>(location, static_cast<Value>(retVal));
  ...
  auto maybeEngine =
      mlir::decisionforest::InferenceRunner::CreateExecutionEngine(module);
  auto &engine = maybeEngine.get();

  // Memref<InputElementType, 2> resultMemref;
  std::vector<TestTileType> tiles = {
  ...
  int32_t returnVal = -1;
  void *funcArgs[] = {&ptr,       &alignedPtr,       &offset,
                      &length,    &stride, // Input memref fields
                      &resultPtr, &resultAlignedPtr, &offset,
                      &resultLen, &stride, // Result memref fields
                      &returnVal};
...
bool Test_LoadTileFeatureIndicesOp_DoubleInt32_TileSize1(TestArgs_t &args) {
  ...
  auto module = mlir::ModuleOp::create(
      location,
      llvm::StringRef("Test_LoadTileFeatureIndicesOp_DoubleInt32_TileSize1"));
  ...
  auto inputMemref = func.getArgument(0);
  auto outputMemref = func.getArgument(1);
  auto featureIndex = builder.create<decisionforest::LoadTileFeatureIndicesOp>(
      location, builder.getI32Type(), static_cast<Value>(inputMemref),
      static_cast<Value>(i), static_cast<Value>(zeroConst));

  builder.create<memref::StoreOp>(location, TypeRange({}),
                                  static_cast<Value>(featureIndex),
                                  outputMemref, i);

  builder.setInsertionPointAfter(batchLoop);
  auto retVal = builder.create<mlir::arith::ConstantIntOp>(
      location, 0, builder.getI32Type());
  builder.create<mlir::func::ReturnOp>(location, static_cast<Value>(retVal));
...
bool Test_LoadTileThresholdOp_Subview_DoubleInt32_TileSize1(TestArgs_t &args) {
  ...
  auto location = builder.getUnknownLoc();
  auto module = mlir::ModuleOp::create(
      location, llvm::StringRef(
                    "Test_LoadTileThresholdOp_Subview_DoubleInt32_TileSize1"));
  ...
  auto memrefSubview = builder.create<memref::SubViewOp>(
      location, inputMemref,
      ArrayRef<OpFoldResult>(builder.getIndexAttr(offset)),
      ArrayRef<OpFoldResult>(builder.getIndexAttr(length - offset)),
      ArrayRef<OpFoldResult>(builder.getIndexAttr(1)));

  auto lengthConstant =
      builder.create<arith::ConstantIndexOp>(location, length - offset);
  auto zeroConst = builder.create<arith::ConstantIndexOp>(location, 0);
  auto oneIndexConst = builder.create<arith::ConstantIndexOp>(location, 1);
  auto batchLoop = builder.create<scf::ForOp>(
      location, zeroConst, lengthConstant,
  ...
  auto threshold = builder.create<decisionforest::LoadTileThresholdsOp>(
      location, builder.getF64Type(), static_cast<Value>(memrefSubview),
      static_cast<Value>(i), static_cast<Value>(zeroConst));
  ...
  int64_t resultLen = length;
  int32_t returnVal = -1;
  void *funcArgs[] = {&ptr,       &alignedPtr,       &memrefOffset,
                      &length,    &stride, // Input memref fields
                      &resultPtr, &resultAlignedPtr, &memrefOffset,
                      &resultLen, &stride, // Result memref fields
                      &returnVal};
  auto invocationResult = engine->invokePacked("TestFunction", funcArgs);
  if (invocationResult) {
...
bool Test_LoadTileFeatureIndicesOp_Subview_DoubleInt32_TileSize1(
  ...
  auto featureIndex = builder.create<decisionforest::LoadTileFeatureIndicesOp>(
      location, builder.getI32Type(), static_cast<Value>(memrefSubview),
      static_cast<Value>(i), static_cast<Value>(zeroConst));
  ...
  decisionforest::TreeEnsembleType GetEnsembleType() override {
    ...
    for (size_t i = 0; i < this->m_forest->NumTrees(); ++i) {
      auto treeType = mlir::decisionforest::TreeType::get(
          GetMLIRType(ReturnType(), this->m_builder),
          m_tilingDescriptors[i].MaxTileSize(),
    ...
    auto forestType = mlir::decisionforest::TreeEnsembleType::get(
        GetMLIRType(ReturnType(), this->m_builder), this->m_forest->NumTrees(),
        this->GetInputRowType(), mlir::decisionforest::ReductionType::kAdd,
        treeTypes);
    return forestType;
  }
...
public:
  ...
  void ConstructForest() override {
    ...
    for (size_t i = 0; i < this->m_forest->NumTrees(); ++i) {
      ...
      if (m_probabilisticTiled) {
        this->m_forest->GetTree(i).GetTiledTree()->SetProbabilisticallyTiled(
            m_probabilisticTiled);
        this->m_forest->GetTree(i).GetTiledTree()->SetLevelsToUnroll(
            m_levelsToUnroll);
      }
    }
    this->m_forest->SetPredictionTransformation(
        decisionforest::PredictionTransformation::kIdentity);
  }

  // Add a function that takes a memref of appropriate type and copies the
  // threshold values from the model into the argument memref.
  void AddThresholdGetter() {
    ...
    auto tileType = decisionforest::TiledNumericalNodeType::get(
        GetMLIRType(ThresholdType(), builder),
        GetMLIRType(FeatureIndexType(), builder), tileShapeType, tileSize);

    auto inputMemrefType = MemRefType::get(shape, tileType);
    auto outputMemrefType =
        MemRefType::get(shape, GetMLIRType(ThresholdType(), builder));
    auto featureIndexMemrefType =
        MemRefType::get(shape, GetMLIRType(FeatureIndexType(), builder));
    auto tileShapeIDMemrefType = MemRefType::get(shape, tileShapeType);
    auto functionType =
        builder.getFunctionType({inputMemrefType, outputMemrefType,
                                 featureIndexMemrefType, tileShapeIDMemrefType},
                                builder.getI32Type());

    auto func = builder.create<mlir::func::FuncOp>(
        location, std::string("Get_ModelValues"), functionType);
    func.setPublic();
    auto &entryBlock = *(func.addEntryBlock());
    builder.setInsertionPointToStart(&entryBlock);
    ...
    auto thresholdVectorType = mlir::VectorType::get(
        {tileSize}, GetMLIRType(ThresholdType(), this->m_builder));
    auto threshold = builder.create<decisionforest::LoadTileThresholdsOp>(
        location, thresholdVectorType, static_cast<Value>(inputMemref),
        static_cast<Value>(i), static_cast<Value>(zeroConst));
    auto featureIndexVectorType = mlir::VectorType::get(
        {tileSize}, GetMLIRType(FeatureIndexType(), this->m_builder));
    auto index = builder.create<decisionforest::LoadTileFeatureIndicesOp>(
        location, featureIndexVectorType, static_cast<Value>(inputMemref),
        static_cast<Value>(i), static_cast<Value>(zeroConst));

    auto tileShape = builder.create<decisionforest::LoadTileShapeOp>(
        location, tileShapeType, static_cast<Value>(inputMemref),
        static_cast<Value>(i), static_cast<Value>(zeroConst));

    auto tileSizeConst =
        builder.create<arith::ConstantIndexOp>(location, tileSize);
    auto outputMemrefOffset = builder.create<mlir::arith::MulIOp>(
        location, builder.getIndexType(), i, tileSizeConst);
    for (int32_t j = 0; j < tileSize; ++j) {
      auto tileIndex = builder.create<arith::ConstantIndexOp>(location, j);
      auto outputMemrefIndex = builder.create<arith::AddIOp>(
          location, builder.getIndexType(), outputMemrefOffset, tileIndex);
      auto jConst = builder.create<arith::ConstantIntOp>(location, j,
                                                         builder.getI32Type());
      auto element =
          builder.create<vector::ExtractElementOp>(location, threshold, jConst);
      builder.create<memref::StoreOp>(location, static_cast<Value>(element),
                                      static_cast<Value>(outputMemref),
                                      static_cast<Value>(outputMemrefIndex));
      auto indexElement =
          builder.create<vector::ExtractElementOp>(location, index, jConst);
      builder.create<memref::StoreOp>(location,
                                      static_cast<Value>(indexElement),
                                      static_cast<Value>(featureIndexMemref),
                                      static_cast<Value>(outputMemrefIndex));
    }
    builder.create<memref::StoreOp>(location, static_cast<Value>(tileShape),
                                    static_cast<Value>(tileShapeMemref), i);
    builder.setInsertionPointAfter(batchLoop);
    auto retVal = builder.create<mlir::arith::ConstantIntOp>(
        location, 0, builder.getI32Type());
    builder.create<mlir::func::ReturnOp>(location, static_cast<Value>(retVal));
...
template <typename ThresholdType = double, typename ReturnType = double,
          ...
          typename InputElementType = double, typename TileShapeType = int32_t>
bool Test_TiledCodeGeneration_SingleTreeModels_BatchSize1(
    TestArgs_t &args, ForestConstructor_t forestConstructor, int32_t tileSize,
    ...
    bool probabilisticTiling = false, int32_t levelsToUnroll = -1) {
  std::vector<decisionforest::TreeTilingDescriptor> tilingDescriptors;
  for (auto &tileIDs : tileIDsVec) {
    decisionforest::TreeTilingDescriptor tilingDescriptor(
        tileSize, 5, tileIDs, decisionforest::TilingType::kRegular);
  ...
  FixedTiledTreeIRConstructor<ThresholdType, ReturnType, FeatureIndexType,
                              NodeIndexType, InputElementType>
      irGenerator(context, serializer, 1, forestConstructor, tilingDescriptors,
                  tileShapeBitWidth);
  ...
  decisionforest::LowerEnsembleToMemrefs(context, module, serializer,
                                         representation);
  ...
  for (auto &row : inputData) {
    ...
    inferenceRunner.RunInference<InputElementType, ReturnType>(inputRow.data(),
                                                               &result);
...
bool Test_TiledCodeGeneration_BalancedTree_BatchSize1(TestArgs_t &args) {
  ...
  std::vector<int32_t> tileIDs_TileSize2 = {0, 0, 1, 2, 5, 3, 4};
  {
    ...
    Test_ASSERT((Test_TiledCodeGeneration_SingleTreeModels_BatchSize1<
                 FPType, FPType, IntType, IntType, FPType>(
        args, forestConstructor, 2, {tileIDs_TileSize2}, childIndexBitWidth)));
    Test_ASSERT((Test_TiledCodeGeneration_SingleTreeModels_BatchSize1<
                 FPType, FPType, IntType, IntType, FPType>(
        args, forestConstructor, 3, {tileIDs}, childIndexBitWidth)));
    Test_ASSERT((Test_TiledCodeGeneration_SingleTreeModels_BatchSize1<
                 FPType, FPType, IntType, IntType, FPType>(
        args, forestConstructor, 4, {tileIDs}, childIndexBitWidth)));
  }
  {
    using FPType = double;
...
bool Test_TiledCodeGeneration_ForestConstructor_BatchSize1(
    TestArgs_t &args, ForestConstructor_t forestConstructor,
    const std::vector<std::vector<int32_t>> &tileIDs,
    int32_t childIndexBitWidth = 1,
    ScheduleManipulator_t scheduleManipulator = nullptr) {
  {
    using FPType = double;
    using IntType = int32_t;
    Test_ASSERT((Test_TiledCodeGeneration_SingleTreeModels_BatchSize1<
                 FPType, FPType, IntType, IntType, FPType, TileShapeType>(
        args, forestConstructor, 2, tileIDs, childIndexBitWidth,
        scheduleManipulator)));
    Test_ASSERT((Test_TiledCodeGeneration_SingleTreeModels_BatchSize1<
                 FPType, FPType, IntType, IntType, FPType, TileShapeType>(
        args, forestConstructor, 3, tileIDs, childIndexBitWidth,
...
bool Test_TiledCodeGeneration_LeftAndRightHeavy_BatchSize1(TestArgs_t &args) {
  std::vector<std::vector<int32_t>> tileIDs = {{0, 0, 1, 2, 3},
                                               {0, 0, 1, 2, 3}};
  auto forestConstructor = AddRightAndLeftHeavyTrees<DoubleInt32Tile>;
  Test_ASSERT((Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, forestConstructor, tileIDs)));
  return true;
}
...
bool Test_TiledCodeGeneration_LeftAndRightHeavy_BatchSize1_SplitTreeLoop(
  ...
  auto forestConstructor = AddRightAndLeftHeavyTrees<DoubleInt32Tile>;
  Test_ASSERT((Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, forestConstructor, tileIDs, 1, SplitTreeDimensionSchedule<1>)));
  return true;
}
...
bool Test_TiledCodeGeneration_LeftAndRightHeavy_BatchSize1_Int8TileSize(
  ...
  std::vector<std::vector<int32_t>> tileIDs = {{0, 0, 1, 2, 3},
                                               {0, 0, 1, 2, 3}};
  auto forestConstructor = AddRightAndLeftHeavyTrees<DoubleInt32Tile>;
  Test_ASSERT((Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int8_t>(
      args, forestConstructor, tileIDs)));
  return true;
}
...
bool Test_TiledCodeGeneration_LeftAndRightHeavy_BatchSize1_Int16TileSize(
  ...
  std::vector<std::vector<int32_t>> tileIDs = {{0, 0, 1, 2, 3},
                                               {0, 0, 1, 2, 3}};
  auto forestConstructor = AddRightAndLeftHeavyTrees<DoubleInt32Tile>;
  Test_ASSERT((Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int16_t>(
      args, forestConstructor, tileIDs)));
  return true;
}
...
bool Test_TiledCodeGeneration_LeftHeavy_BatchSize1(TestArgs_t &args) {
  ...
  return Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs});
}
...
bool Test_TiledCodeGeneration_RightHeavy_BatchSize1(TestArgs_t &args) {
  std::vector<int32_t> tileIDs = {
      0, 0, 1, 2, 3}; // The root and one of its children are in one tile and
                      // all leaves are in separate tiles
  return Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, AddRightHeavyTree<DoubleInt32Tile>, {tileIDs});
}
...
bool Test_TiledCodeGeneration_LeftHeavy_BatchSize1_Int8TileShape(
  ...
  std::vector<int32_t> tileIDs = {
      0, 0, 1, 2, 3}; // The root and one of its children are in one tile and
                      // all leaves are in separate tiles
  return Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int8_t>(
      args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs});
}
...
bool Test_TiledCodeGeneration_LeftHeavy_BatchSize1_Int16TileShape(
  ...
  std::vector<int32_t> tileIDs = {
      0, 0, 1, 2, 3}; // The root and one of its children are in one tile and
                      // all leaves are in separate tiles
  return Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int16_t>(
      args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs});
}
...
bool Test_SparseTiledCodeGeneration_LeftAndRightHeavy_BatchSize1(
  ...
  auto forestConstructor = AddRightAndLeftHeavyTrees<DoubleInt32Tile>;
  Test_ASSERT((Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, forestConstructor, tileIDs, 32)));
  return true;
}
...
bool Test_SparseTiledCodeGeneration_LeftHeavy_BatchSize1(TestArgs_t &args) {
  ...
  std::vector<int32_t> tileIDs = {
      0, 0, 1, 2, 3}; // The root and one of its children are in one tile and
                      // all leaves are in separate tiles
  return Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs}, 32);
}
...
bool Test_SparseTiledCodeGeneration_RightHeavy_BatchSize1(TestArgs_t &args) {
  ...
  std::vector<int32_t> tileIDs = {
      0, 0, 1, 2, 3}; // The root and one of its children are in one tile and
                      // all leaves are in separate tiles
  return Test_TiledCodeGeneration_ForestConstructor_BatchSize1<int32_t>(
      args, AddRightHeavyTree<DoubleInt32Tile>, {tileIDs}, 32);
}
...
template <typename ThresholdType, typename ReturnType,
          typename FeatureIndexType, typename NodeIndexType,
          typename InputElementType, int32_t tileSize,
          typename TileShapeType = int32_t>
bool Test_ModelInitialization(
    TestArgs_t &args, ForestConstructor_t forestConstructor,
    const std::vector<std::vector<int32_t>> &tileIDsVec) {
  std::vector<decisionforest::TreeTilingDescriptor> tilingDescriptors;
  for (auto &tileIDs : tileIDsVec) {
    decisionforest::TreeTilingDescriptor tilingDescriptor(
        tileSize, -1, tileIDs, decisionforest::TilingType::kRegular);
    tilingDescriptors.push_back(tilingDescriptor);
  }
  ...
  mlir::decisionforest::ForestJSONReader::GetInstance().GetModelValues(
      tileSize, thresholdSize, featureIndexSize, thresholds, featureIndices,
      tileShapeIDs);
  ...
  decisionforest::Memref<ThresholdType, 1> thresholdMemref{
      thresholdsInMLIRModule.data(), thresholdsInMLIRModule.data(), 0,
      (int64_t)thresholds.size(), 1};

  std::vector<FeatureIndexType> featureIndicesInMLIRModule(
      featureIndices.size(), -1);
  decisionforest::Memref<FeatureIndexType, 1> featureIndicesMemref{
      featureIndicesInMLIRModule.data(), featureIndicesInMLIRModule.data(), 0,
      (int64_t)featureIndicesInMLIRModule.size(), 1};

  std::vector<TileShapeType> tileShapeIDsInMLIRModule(tileShapeIDs.size(), -1);
  decisionforest::Memref<TileShapeType, 1> tileShapeIDsMemref{
      tileShapeIDsInMLIRModule.data(), tileShapeIDsInMLIRModule.data(), 0,
      (int64_t)tileShapeIDsInMLIRModule.size(), 1};

  {
    int32_t retVal = -1;
    // Get the threshold values from the model memref
    std::vector<void *> args = {&modelMemref.bufferPtr,
                                ...
                                &tileShapeIDsMemref.bufferPtr,
                                &tileShapeIDsMemref.alignedPtr,
                                &tileShapeIDsMemref.offset,
                                &tileShapeIDsMemref.lengths[0],
                                &tileShapeIDsMemref.strides[0],
                                &retVal};
...
bool Test_ModelInit_LeftHeavy(TestArgs_t &args) {
                      ...
                      // all leaves are in separate tiles
  {
    using FPType = double;
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 2>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 3>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 4>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int16_t, int16_t, FPType, 2>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int16_t, int16_t, FPType, 3>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int16_t, int16_t, FPType, 4>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int8_t, int8_t, FPType, 2>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int8_t, int8_t, FPType, 3>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int8_t, int8_t, FPType, 4>(
            args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
  }
  {
    using FPType = float;
...
bool Test_ModelInit_LeftHeavy_Int8TileShape(TestArgs_t &args) {
  ...
  {
    using FPType = double;
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int32_t, int32_t,
                                          FPType, 2, TileShapeType>(
        args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int32_t, int32_t,
                                          FPType, 3, TileShapeType>(
        args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int32_t, int32_t,
                                          FPType, 4, TileShapeType>(
        args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int16_t, int16_t,
                                          FPType, 2, TileShapeType>(
        args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int16_t, int16_t,
                                          FPType, 3, TileShapeType>(
    ...
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int16_t, int16_t,
                                          FPType, 4, TileShapeType>(
        args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int8_t, int8_t,
                                          FPType, 2, TileShapeType>(
        args, AddLeftHeavyTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT((Test_ModelInitialization<FPType, FPType, int8_t, int8_t,
                                          FPType, 3, TileShapeType>(
...
bool Test_ModelInit_Balanced(TestArgs_t &args) {
  ...
  std::vector<int32_t> tileIDs_TileSize2 = {0, 0, 1, 2, 5, 3, 4};
  {
    ...
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 2>(
            args, AddBalancedTree<DoubleInt32Tile>, {tileIDs_TileSize2})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 3>(
            args, AddBalancedTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 4>(
            args, AddBalancedTree<DoubleInt32Tile>, {tileIDs})));
    Test_ASSERT(
...
bool Test_ModelInit_RightAndLeftHeavy(TestArgs_t &args) {
  std::vector<std::vector<int32_t>> tileIDs = {
      {0, 0, 1, 2, 3},
      {0, 0, 1, 2, 3}}; // The root and one of its children are in one tile and
                        // all leaves are in separate tiles
  {
    ...
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 2>(
            args, AddRightAndLeftHeavyTrees<DoubleInt32Tile>, tileIDs)));
    Test_ASSERT(
        (Test_ModelInitialization<FPType, FPType, int32_t, int32_t, FPType, 3>(
            args, AddRightAndLeftHeavyTrees<DoubleInt32Tile>, tileIDs)));
    Test_ASSERT(
...
bool Test_UniformTiling_BatchSize1(TestArgs_t &args,
  ...
  FixedTreeIRConstructor<ThresholdType, ReturnType, FeatureIndexType,
                         NodeIndexType, InputElementType>
      irGenerator(context, serializer, 1, forestConstructor);
  irGenerator.ConstructForest();
  auto module = irGenerator.GetEvaluationFunction();
  decisionforest::DoUniformTiling(context, module, tileSize, tileShapeBitWidth,
...
bool Test_UniformTiling_BatchSize1_AllTypes(
    TestArgs_t &args, ForestConstructor_t forestConstructor,
    int32_t tileShapeBitWidth, bool makeAllLeavesSameDepth = false) {
  {
    using FPType = double;
    using IntType = int32_t;
    Test_ASSERT((
        Test_UniformTiling_BatchSize1<FPType, FPType, IntType, IntType, FPType>(
            args, forestConstructor, 2, tileShapeBitWidth,
            makeAllLeavesSameDepth)));
    Test_ASSERT((
        Test_UniformTiling_BatchSize1<FPType, FPType, IntType, IntType, FPType>(
            args, forestConstructor, 3, tileShapeBitWidth,
            makeAllLeavesSameDepth)));
    Test_ASSERT((
        Test_UniformTiling_BatchSize1<FPType, FPType, IntType, IntType, FPType>(
            args, forestConstructor, 4, tileShapeBitWidth,
            makeAllLeavesSameDepth)));
  }
  {
    using FPType = double;
