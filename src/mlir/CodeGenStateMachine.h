...
public:
  ...
  virtual bool EmitNext(ConversionPatternRewriter &rewriter,
                        Location &location) = 0;
...
private:
  std::vector<std::unique_ptr<ICodeGeneratorStateMachine>>
      m_codeGenStateMachines;
  bool m_startedEmitting = false;
  bool m_finishedEmitting = false;

public:
  InterleavedCodeGenStateMachine(){};
  InterleavedCodeGenStateMachine(const InterleavedCodeGenStateMachine &) =
      delete;
  InterleavedCodeGenStateMachine &
  operator=(const InterleavedCodeGenStateMachine &) = delete;

  void
  AddStateMachine(std::unique_ptr<ICodeGeneratorStateMachine> codeGenerator) {
  ...
  virtual std::vector<Value> GetResult() override {
    ...
    for (auto &codeGenerator : m_codeGenStateMachines) {
      ...
      results.insert(results.end(), std::make_move_iterator(result.begin()),
  ...
  ~InterleavedCodeGenStateMachine() { m_codeGenStateMachines.clear(); }
};
...
public:
  ScalarTraverseTileCodeGenerator(
  ...
  bool EmitNext(ConversionPatternRewriter &rewriter,
                Location &location) override;
...
public:
  VectorTraverseTileCodeGenerator(
...
private:
  ...
  Value m_nodeToTraverseIndex;

public:
  GPUVectorTraverseTileCodeGenerator(
      decisionforest::CooperativeTraverseTreeTileOp traverseTileOp,
