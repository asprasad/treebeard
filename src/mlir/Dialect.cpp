...
void TreeEnsembleTypeStorage::print(mlir::DialectAsmPrinter &printer) {
  std::string ensembleType =
      m_ensembleSubset ? "TreeEnsembleSubsetType" : "TreeEnsembleType";
  printer << ensembleType << "(#Trees:" << m_numTrees
          << ", rowType:" << m_rowType << ", resultType:" << m_resultType
          << ", reductionType:" << (int32_t)m_reductionType << ")";
}
...
::mlir::Type
DecisionForestDialect::parseType(::mlir::DialectAsmParser &parser) const {
  llvm_unreachable("DecisionForestDialect::parseAttribute Unimplement");
  return ::mlir::Type();
}

// TODO Can this somehow be made polymorphic? MLIR seems to pass everything by
// value!
...
void DecisionForestDialect::printType(::mlir::Type type,
  ...
  if (type.isa<mlir::decisionforest::NodeType>()) {
    mlir::decisionforest::NodeType nodeType =
        type.cast<mlir::decisionforest::NodeType>();
    nodeType.print(os);
  } else if (type.isa<mlir::decisionforest::TreeEnsembleType>()) {
    mlir::decisionforest::TreeEnsembleType ensembleType =
        type.cast<mlir::decisionforest::TreeEnsembleType>();
    ensembleType.print(os);
  } else if (type.isa<mlir::decisionforest::TreeType>()) {
    mlir::decisionforest::TreeType treeType =
        type.cast<mlir::decisionforest::TreeType>();
    treeType.print(os);
  } else if (type.isa<mlir::decisionforest::NumericalNodeType>()) {
    auto numericalNodeType =
        type.cast<mlir::decisionforest::NumericalNodeType>();
    ...
    auto tiledNodeType =
        type.cast<mlir::decisionforest::TiledNumericalNodeType>();
    ...
    auto reorgMemrefElementType =
        type.cast<mlir::decisionforest::ReorgMemrefElementType>();
...
void DecisionForestDialect::printAttribute(
    ::mlir::Attribute attr, ::mlir::DialectAsmPrinter &os) const {
  if (attr.isa<DecisionForestAttribute>()) {
    DecisionForestAttribute decisionForrestAttr =
        attr.cast<DecisionForestAttribute>();
