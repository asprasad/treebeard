...
struct EnsembleConstantOpLowering: public ConversionPattern {
  ...
  void AddModelMemrefInitFunction(mlir::ModuleOp module, std::string globalName, MemRefType memrefType, 
    ...
    auto initMemrefFuncType = rewriter.getFunctionType(TypeRange{thresholdArgType, indexArgType, tileShapeIDArgType, childrenIndexArgType}, rewriter.getI32Type());
  ...
  std::tuple<Type, Type, Type> AddGlobalMemrefs(mlir::ModuleOp module, mlir::decisionforest::EnsembleConstantOp& ensembleConstOp,
    ...
    // assert (tileSize == 1);
    Type memrefElementType = decisionforest::TiledNumericalNodeType::get(thresholdType, featureIndexType, tileShapeType, tileSize, true, childIndexType);
...
struct GetTreeOpLowering: public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const final {
    ...
    auto leavesMemref = rewriter.create<memref::SubViewOp>(location, ensembleInfo.leavesGlobal, ArrayRef<OpFoldResult>({static_cast<Value>(leavesMemrefIndex)}),
...
struct IsLeafOpLowering: public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const final {
    ...
    auto loopCondition = rewriter.create<arith::OrIOp>(location, nodeIndexOutOfBounds, comparison);
    rewriter.replaceOp(op, static_cast<Value>(loopCondition));
...
struct TraverseTreeTileOpLowering : public ConversionPattern {
  ...
  void LowerOpTileSize1(Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const {
    ...
    auto newIndex = rewriter.create<arith::AddIOp>(location, rewriter.getIndexType(), static_cast<Value>(loadChildIndex), static_cast<Value>(comparisonResultIndex));
...
struct GetLeafValueOpLowering : public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const final {
    ...
    auto ifElse = rewriter.create<scf::IfOp>(location, TypeRange{ thresholdType }, nodeIndexOutOfBounds, true);
    {
