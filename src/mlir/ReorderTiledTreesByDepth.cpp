...
struct SplitTreeLoopsByTreeDepthPattern : public RewritePattern {
  ...
  void SplitTreeLoopForProbAndUniformTiling(
      ...
      decisionforest::IndexVariable *&probTreeIndex,
      decisionforest::IndexVariable *&probBatchIndex,
      decisionforest::IndexVariable *&unifTreeIndex,
      decisionforest::IndexVariable *&unifBatchIndex,
      decisionforest::IndexVariable *currentTreeIndex,
      decisionforest::IndexVariable *currentBatchIndex) const {
    ...
    if (!forest.GetTree(0).GetTiledTree()->IsProbabilisticallyTiled()) {
      ...
      return;
    } else if (forest.GetTrees()
                   .back()
                   ->GetTiledTree()
                   ->IsProbabilisticallyTiled()) {
    ...
    assert(splitPoint != 0 &&
           "Expected at least one tree to be probabilistically tiled");
    ...
    schedule->Split(*currentTreeIndex, *probTreeIndex, *unifTreeIndex,
                    splitPoint, indexMap);
  ...
  void SplitTreeLoopForProbabilityBasedTiling(
      decisionforest::Schedule *schedule,
      decisionforest::DecisionForest &forest,
      decisionforest::IndexVariable *batchIndexPtr,
    ...
    while (currTreeIndex < (int32_t)treeIndex.GetRange().m_stop) {
      ...
      std::map<decisionforest::IndexVariable *,
          ...
          indexMap;
      while (currDepth == intervalEndTreeDepth &&
             ++intervalEnd < indexToSplit->GetRange().m_stop) {
        intervalEndTreeDepth =
            forest.GetTree(intervalEnd).GetTiledTree()->GetLevelsToUnroll();
      }
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (m_numberOfCores != -1) {
      ...
      schedule->Tile(batchIndex, b0_parallel, b1,
                     schedule->GetBatchSize() / m_numberOfCores);
    ...
    decisionforest::IndexVariable *probTreeIndex = nullptr,
                                  *probBatchIndex = nullptr;
    decisionforest::IndexVariable *unifTreeIndex = nullptr,
                                  *unifBatchIndex = nullptr;
    SplitTreeLoopForProbAndUniformTiling(
        schedule, forest, probTreeIndex, probBatchIndex, unifTreeIndex,
        unifBatchIndex, &treeIndex, batchIndexPtr);
...
struct SplitTreeLoopByDepth
  ...
  void runOnOperation() final {
    RewritePatternSet patterns(&getContext());
    patterns.add<SplitTreeLoopsByTreeDepthPattern>(
        &getContext(), m_pipelineSize, m_numCores, m_parallelTreeBatches);
