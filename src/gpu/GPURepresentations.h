...
public:
  ...
  // mlir::Value GenerateGetTreeClassId(mlir::ConversionPatternRewriter
  // &rewriter, mlir::Operation *op, Value ensemble, Value treeIndex) override;
...
protected:
  ...
  Type GenerateLeafBuffers(ConversionPatternRewriter &rewriter,
                           Location location, ModuleOp module, Operation *op,
                           std::vector<Type> &cleanupArgs);
  Value GenerateLeavesBufferCaching(
      ConversionPatternRewriter &rewriter, mlir::Operation *op,
      ArrayRef<Value> operands,
